...
architecture project_tb_arch of project_tb is
    ...
    signal scenario_input_2 : scenario_type := (51,  0,  0,  0, 57,  0, 24,  0,  0,  0, 24,  0, 126,  0,   0,  0, 192,  0,   0,  0);
	signal scenario_full_2  : scenario_type := (51, 31, 51, 30, 57, 31, 24, 31, 24, 30, 24, 31, 126, 31, 126, 30, 192, 31, 192, 30);  
    constant SCENARIO_ADDRESS_2 : integer := 300;
...
begin
    ...
    begin
        ...
        tb_start <= '1'; --- D? il segnale di partenza al componente
        wait for 600 ns; --Dopo un tempo casuale arriva segnale di reset asincrono
        tb_rst <= '1'; -- Segnale di reset casuale
            --- Fine Sessione 1 ---
        wait for 20 ns; 
        tb_rst <= '0';
        wait for 20 ns;
    ...
    begin
	    ...
	    --Rifaccio il controllo precedente dopo il secondo segnale di reset
	    wait until tb_rst = '1';
        wait for 25 ns;
        assert tb_done = '0' report "TEST FALLITO o_done !=0 during reset" severity failure;
        wait until tb_rst = '0';
        ...
        assert false report "Session 1 terminated! TEST 1/2 PASSED" severity note;
			--- Fine Sessione 1 ---
			--- Sessione 2 ---
        ...
        for i in 0 to SCENARIO_LENGTH_2*2-1 loop
            assert RAM(SCENARIO_ADDRESS_2+i) = std_logic_vector(to_unsigned(scenario_full_2(i),8)) report "TEST_2 FALLITO @" & integer'image(i) & " expected= " & integer'image(scenario_full_2(i)) & " actual=" & integer'image(to_integer(unsigned(RAM(SCENARIO_ADDRESS_2+i)))) severity failure;
        end loop;
        ...
        assert false report "Session 2 terminated! TEST 2/2 PASSED" severity note;
	       --- Fine Sessione 2 ---
	
        assert false report "Simulation Ended! TEST PASSATO (OTHERS_RESET_AND_RESUME)" severity failure;
    end process;
